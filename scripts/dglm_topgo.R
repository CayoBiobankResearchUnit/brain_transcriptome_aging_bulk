...
dglm.sbet = dglm.beta / sqrt(dglm.results[,'bvar',])
...
# 
# go.enrichment.results = rbind(all.region.results,one.region.results,single.region.results)
...
# # The function below was originally parallelized with mclapply. Turned this off to avoid conflicts in the writing/rewriting of topGO methods.
# # dglm.region.results = do.call(rbind,mclapply(names(keep.genes),function(i) {
# dglm.region.results = do.call(rbind,lapply(names(keep.genes),function(i) {
# 
# #out = vector(mode='list',length=length(keep.genes))
# #names(out) = names(keep.genes)
# #for (i in names(keep.genes)) {
# #	message(i)
# 
# 
# 	this.region.fet = this.region.kst = numeric(length=length(ensembl.gene.names))
...
# 
# 	# Code upregulated genes as 1 and downregulated genes as -1 if they are significant and in the noted direction
# 	this.region.fet[names(which(unlist(lapply(dglm.genes,function(x) {
# 		dglm.qval[x,i] < fsr.cutoff & dglm.beta[x,i] > 0
# 	}))))] = 1
# 	this.region.fet[names(which(unlist(lapply(dglm.genes,function(x) {
# 		dglm.qval[x,i] < fsr.cutoff & dglm.beta[x,i] < 0
# 	}))))] = -1
# 
# 	this.region.kst[rownames(dglm.sbet)] = dglm.sbet[,i]
# 
# 	# Set topGO methods back to the originals (saving p-values into the scores S4 slot)
# 	setMethod('GOKSTest',signature(c(object='classicScore')),get.ks.pval)
# 	setMethod('GOFisherTest',signature(c(object='classicCount')),get.fisher.pval)
...
# 		description='Simple session',
# 		ontology='BP',
# 		allGenes=-this.region.kst[!is.na(this.region.kst)],
# 		geneSelectionFun=function(x) x > 0,
# 		nodeSize = 10,
# 		annotationFun = annFUN.gene2GO,
# 		gene2GO = gene2go
...
# 	
# 	
# #	out[[i]] = rbind(this.region.fet.inc.results,this.region.fet.dec.results,this.region.kst.inc.results,this.region.kst.dec.results)
# #}
# }))
# # },mc.cores=n.cores))
# 
# rownames(dglm.region.results) = NULL
# 
# go.enrichment.results$method = 'MASH'
# dglm.region.results$dataset = 'GO'
# dglm.region.results$method = 'DGLM'
# 
# go.enrichment.results = rbind(go.enrichment.results,dglm.region.results)



saveRDS(go.enrichment.results,file='checkpoints/dglm_topgo_results.rds')
...
saveRDS(go.specificity.results,file='checkpoints/dglm_go_specificity_results.rds')